...
from lowkeyMusix.config import ARQ_API_KEY
...
async def hfmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        lel = await message.reply("`sabar boty!...`")
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        title = file_name
        thumb_name = "https://telegra.ph/file/68af50ef043dd26466c84.png"
        ...
        query = toxt
        await lel.edit("🎵 <b>bisa sabar ga sih boty!🖕🏻</b>")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in DISABLED_GROUPS:
        return    
    lel = await message_.reply("🔄 <b>sabar lagi di proses!💋</b>")
    administrators = await get_administrators(message_.chat)
    chid = message_.chat.id
    try:
        user = await USER.get_me()
    except:
        user.first_name = "lowkeymusic"
    usar = user
    wew = usar.id
    try:
        # chatdetails = await USER.get_chat(chid)
            ...
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        ...
                        "\n\nOr manually add @callmesgc to your Group and try again</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        await cb.answer("lu siapa sih nyuruh gw mulu!", show_alert=True)
        return
    await cb.message.edit("SABAR BOTY...BENTAR LAGI GW NAIK NYANYI")
    x=int(x)
    try:
